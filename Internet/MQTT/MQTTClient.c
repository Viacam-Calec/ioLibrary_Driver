...
{
	return c->next_packetid = (c->next_packetid == MAX_PACKET_ID) ?
					  1 :
					  c->next_packetid + 1;
}
...
static int readPacket(MQTTClient *c, Timer *timer)
{
	...
	if (c->ipstack->mqttread(c->ipstack, c->readbuf, 1,
				 TimerLeftMS(timer)) != 1)
		goto exit;
	...
	if (rem_len > 0 &&
	    (c->ipstack->mqttread(c->ipstack, c->readbuf + len, rem_len,
...
int keepalive(MQTTClient *c)
{
	...
	if (TimerIsExpired(&c->ping_timer)) {
		if (!c->ping_outstanding) {
			...
			if (len > 0 && (rc = sendPacket(c, len, &timer)) ==
					       SUCCESSS) // send the ping packet
...
int cycle(MQTTClient *c, Timer *timer)
{
	...
	case PUBLISH: {
		...
		MQTTMessage msg;
		int intQoS;
		if (MQTTDeserialize_publish(&msg.dup, &intQoS, &msg.retained,
					    &msg.id, &topicName,
					    (unsigned char **)&msg.payload,
					    (int *)&msg.payloadlen, c->readbuf,
					    c->readbuf_size) != 1)
			goto exit;
	...
	case PUBREC: {
		...
		if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf,
					c->readbuf_size) != 1)
			rc = FAILURE;
		else if ((len = MQTTSerialize_ack(c->buf, c->buf_size, PUBREL,
						  0, mypacketid)) <= 0)
			rc = FAILURE;
		else if ((rc = sendPacket(c, len, timer)) !=
			 SUCCESSS) // send the PUBREL packet
...
void MQTTRun(void *parm)
{
	...
	while (1) {
		...
		TimerCountdownMS(
			&timer,
			500); /* Don't wait too long if no traffic is incoming */
		cycle(c, &timer);
#if defined(MQTT_TASK)
		MutexUnlock(&c->mutex);
#endif
...
int MQTTConnect(MQTTClient *c, MQTTPacket_connectData *options)
{
	...
	if (options == 0)
		options =
			&default_options; /* set default options if none were supplied */

	c->keepAliveInterval = options->keepAliveInterval;
	TimerCountdown(&c->ping_timer, c->keepAliveInterval);
	if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0)
		goto exit;
	if ((rc = sendPacket(c, len, &connect_timer)) !=
	    SUCCESSS) // send the connect packet
		goto exit; // there was a problem
		  ...
		  messageHandler messageHandler)
{
	...
	len = MQTTSerialize_subscribe(c->buf, c->buf_size, 0,
				      getNextPacketId(c), 1, &topic, &charQos);
	if (len <= 0)
		goto exit;
	if ((rc = sendPacket(c, len, &timer)) !=
	    SUCCESSS) // send the subscribe packet
		goto exit; // there was a problem
...
int MQTTUnsubscribe(MQTTClient *c, const char *topicFilter)
{
	...
	if (waitfor(c, UNSUBACK, &timer) == UNSUBACK) {
		unsigned short
			mypacketid; // should be the same as the packetid above
